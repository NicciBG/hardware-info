/* Standalone verification tool for libgpu_nvidia_*. See
   tools/gpu_amd_probe.c for the pattern this follows. */

#include "gpu_nvidia.h"

#include <stdio.h>
#include <time.h>

int main(void) {
    const char *out_path = "gpu_nvidia_probe_result.txt";
    FILE *out = fopen(out_path, "w");
    if (!out) {
        fprintf(stderr, "Could not create %s in the current folder.\n", out_path);
        return 1;
    }

    time_t now = time(nullptr);
    fprintf(out, "NVIDIA GPU probe result\n");
    fprintf(out, "generated: %s", ctime(&now));

    GPU_NVIDIA_DATA data;
    int rc = get_nvidia_gpu_data(&data);
    fprintf(out, "get_nvidia_gpu_data return code: %d\n", rc);
    fprintf(out, "nvidia_smi_available: %s\n\n", data.nvidia_smi_available ? "yes" : "no");

    if (rc != 0) {
        fprintf(out, "Call failed (see gpu_nvidia.h for what code %d means).\n", rc);
    } else {
        fprintf(out, "NVIDIA GPUs found: %d\n\n", data.device_count);
        for (int i = 0; i < data.device_count; i++) {
            const GPU_NVIDIA_Device *d = &data.devices[i];
            fprintf(out, "Device %d:\n", i);
            fprintf(out, "  name:              %s\n", d->name);
            fprintf(out, "  uuid:              %s\n", d->uuid);
            fprintf(out, "  pci_bus_location:  %s\n", d->pci_bus_location);
            fprintf(out, "  pci_vendor_id:     0x%04x\n", d->pci_vendor_id);
            fprintf(out, "  pci_device_id:     0x%04x\n", d->pci_device_id);
            fprintf(out, "  vram_total_bytes:  %llu (%.2f GB)\n",
                    (unsigned long long)d->vram_total_bytes,
                    (double)d->vram_total_bytes / 1e9);
            fprintf(out, "  vram_used_bytes:   %llu\n", (unsigned long long)d->vram_used_bytes);
            fprintf(out, "  core_clock_mhz:    %d%s\n", d->core_clock_mhz,
                    d->core_clock_mhz < 0 ? " (not available)" : "");
            fprintf(out, "  memory_clock_mhz:  %d%s\n\n", d->memory_clock_mhz,
                    d->memory_clock_mhz < 0 ? " (not available)" : "");
        }
        free_nvidia_gpu_data(&data);
    }

    fclose(out);

    printf("Done. Wrote %s in this folder.\n", out_path);
    printf("Please send that file back so the results can be checked against your actual GPU.\n");
    return 0;
}
